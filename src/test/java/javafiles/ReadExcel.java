...
//	static HSSFCell cell;
	...
	public static synchronized void setCellHeading(String FPath, HSSFWorkbook wb,
	...
	public static synchronized void setCellValue(String FPath, HSSFWorkbook wb, HSSFSheet ws, String sname, int RowNum, int ColNum, Object[][] data) throws Exception{
		...
		try {
			...
			for (int j = 0; j < data.length; j++) {
				for (int k = 0; k < data[j].length; k++) {		
					...
					Row.getCell(ColNum).setCellValue(data[j][k].toString());

				}
			}
			wb.write(new FileOutputStream(new File(FPath)));
			wb.close();
			return;
	...
	public static synchronized void updateCellValue(String FPath, HSSFWorkbook wb, HSSFSheet ws, String sname, int RowNum, int ColNum, Object[][] data) throws Exception{
	...
	public static synchronized void WriteResult(String file, HSSFWorkbook wb, HSSFSheet ws, String sheetname, String username, String password, String result)throws Exception{

		FileInputStream filein = new FileInputStream(new File(file));
		wb = new HSSFWorkbook(filein);
		ws = wb.getSheet(sheetname);

		Iterator<org.apache.poi.ss.usermodel.Row> rowIterator0 = ws.iterator();
